...
// Builds a fixture DB, runs the candidate as `node <file> <db> <orderId>`
// with stateset-mcp on PATH, then independently re-reads the database.
const root = join(dirname(fileURLToPath(import.meta.url)), '..', '..');
const candidate = resolve(process.argv[2] ?? join(
  dirname(fileURLToPath(import.meta.url)), 'solution.example.mjs',
));
const directory = mkdtempSync(join(tmpdir(), 'benchmark-17-'));
...
try {
  ...
  const reopened = await check.orders.get(order.id);
  assert.equal(reopened.status, 'pending');
  assert.equal(reopened.totalAmountExact, '25.00');
  assert.equal(await check.orders.count(), 1);
  console.log('PASS 17-mcp-order-inspect');
} catch (error) {
  console.error(`FAIL 17-mcp-order-inspect: ${error.message}`);
  process.exitCode = 1;
} finally {
